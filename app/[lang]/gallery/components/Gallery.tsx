...
export default function Gallery({ data }: any) {
  ...
  return (
    <section className="w-full flex flex-col items-center px-6 md:px-8 2xl:px-12">
      <div className="flex flex-col items-center sm:grid sm:grid-cols-2 xl:grid-cols-3 gap-5 max-w-4xl xl:max-w-[1612px] w-full">
        {data.items
      ...
      {currentNumberOfImages < data.items.length && (
        <button
          ...
          className="h-14 mt-12 md:mt-16 2xl:mt-24 flex items-center justify-center min-w-52 px-6 border-2 border-transparent font-bold rounded-lg bg-blue-800 text-white hover:border-blue-800 hover:bg-white hover:text-black"
        >
          <span>{data.loadMoreBtn}</span>
        </button>
      )}
      {lightboxOpen !== null &&
        createPortal(
